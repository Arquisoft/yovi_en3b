...
|===
| **Name** | **Responsibility**
| **REST API Controller** | The HTTP listener that receives YEN-encoded requests from the Webapp via Port 4000.
| **Game Engine Core** | The engine's core that validates Game Y rules and determines game outcomes.
| **Bot Logic/AI** | Manages various AI difficulty levels and provides next-move suggestions.
| **YEN Parser** | Translates between JSON-based YEN notation and internal Rust data structures.
|===

==== 5.2.2 White Box Web Client
The Web Client is the primary interface for users. Its internal architecture separates visual components from communication logic.

image::level-2-webapp.jpg["Web Client White Box Diagram"]
